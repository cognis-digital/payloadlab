"""Native cognis-connect emit for payloadlab — forward findings to any platform.

Maps payloadlab's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    payloadlab ... --format json | payloadlab-emit --to stix
    payloadlab-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop("fmt", None)
        out.pop("size", None)
        out.pop("sha_hint", None)
        out.pop("entropy", None)
        out.pop("score", None)
        out.pop("verdict", None)
        out.pop("rule", None)
        out.pop("evidence", None)
        out.pop("tags", None)
        out["title"] = rec.get("rule", "")
